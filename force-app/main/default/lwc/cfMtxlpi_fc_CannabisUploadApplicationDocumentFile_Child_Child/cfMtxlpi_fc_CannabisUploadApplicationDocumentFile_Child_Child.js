import { FlexCardMixin } from "vlocity_ins/flexCardMixin";
    import {interpolateWithRegex, interpolateKeyValue, fetchCustomLabels, loadCssFromStaticResource } from "vlocity_ins/flexCardUtility";
          ...
          export default class cfMtxlpi_fc_CannabisUploadApplicationDocumentFile_Child_Child extends FlexCardMixin(OmniscriptBaseMixin(LightningElement)){
              @api debug;
              @api recordId;
              @api objectApiName;
              @track _omniSupportKey = 'cfMtxlpi_fc_CannabisUploadApplicationDocumentFile_Child_Child';
              ...
              connectedCallback() {
                ...
                this.setAttribute(
                  "class",
                  "card-a6H5f000000LRMhEAO"
                );
                this.getCommunityPrefix().then((result) => {
                  loadStyle(this, result +"/servlet/servlet.FileDownload?file=00P5f000001912wEAA").then(() =>{
                    //Loading Custom CSS.
                  })
                });
        ...
        pubsub.register(`mtxlpi_fc_CannabisUploadApplicationDocumentFile_Child_Child`,this.pubsubEvent[0]);

              }

              unregisterEvents(){
                pubsub.unregister(`mtxlpi_fc_CannabisUploadApplicationDocumentFile_Child_Child`,this.pubsubEvent[0]);
