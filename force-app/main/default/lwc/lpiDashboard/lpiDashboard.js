...
import getSavedApplications from "@salesforce/apex/bcdDashboardApex.getSavedApplications";
...
export default class LpiDashboard extends LightningElement {
    ...
    connectedCallback() {
            ...
            .then((result) => {
                ...
                for (let i = 0; i < result.length; i++) {
                    if (result[i].Renew == true) {
                        this.ExpiringLicenseCount =
                            this.ExpiringLicenseCount + 1;
                    }
        ...
        getSavedApplications()
            ...
            .catch((error) => {
                console.log("getSavedApplications Error: ", error);
            });
    }
    ...
    onClick(event) {
        ...
        this.renewURL =
            "https://mtxlpi.force.com/tlcPortal/s/license-renewal?licenseId=" +
            event.target.name;
    ...
    onUpdate(event) {
        ...
        window.open(
            "https://mtxlpi.force.com/tlcPortal/s/update-license?licenseId=" +
                event.target.name,
            "_self"
        );
    }
