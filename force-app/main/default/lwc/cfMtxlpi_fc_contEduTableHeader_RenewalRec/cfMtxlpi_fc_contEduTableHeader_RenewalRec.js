import { FlexCardMixin } from "vlocity_ins/flexCardMixin";
    import {interpolateWithRegex, interpolateKeyValue, fetchCustomLabels, loadCssFromStaticResource } from "vlocity_ins/flexCardUtility";
          ...
          export default class cfMtxlpi_fc_contEduTableHeader_RenewalRec extends FlexCardMixin(OmniscriptBaseMixin(LightningElement)){
              @api debug;
              @api recordId;
              @api objectApiName;
              @track _omniSupportKey = 'cfMtxlpi_fc_contEduTableHeader_RenewalRec';
              ...
              connectedCallback() {
                ...
                this.setAttribute(
                  "class",
                  "card-a6H5f000000LRPmEAO"
                );
                this.getCommunityPrefix().then((result) => {
                  loadStyle(this, result +"/servlet/servlet.FileDownload?file=00P5f00000190hNEAQ").then(() =>{
                    //Loading Custom CSS.
                  })
                });
...
data: this.handleEventAction.bind(this, data.events[1],1)
        ...
        pubsub.register(`mtxlpi_fc_contEduTableHeader_RenewalRec`,this.pubsubEvent[0]);

              }

              unregisterEvents(){
                pubsub.unregister(`mtxlpi_fc_contEduTableHeader_RenewalRec`,this.pubsubEvent[0]);
