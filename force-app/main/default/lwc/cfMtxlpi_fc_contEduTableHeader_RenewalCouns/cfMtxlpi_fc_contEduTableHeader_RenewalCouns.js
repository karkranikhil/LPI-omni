import { FlexCardMixin } from "vlocity_ins/flexCardMixin";
    import {interpolateWithRegex, interpolateKeyValue, fetchCustomLabels, loadCssFromStaticResource } from "vlocity_ins/flexCardUtility";
          ...
          export default class cfMtxlpi_fc_contEduTableHeader_RenewalCouns extends FlexCardMixin(OmniscriptBaseMixin(LightningElement)){
              @api debug;
              @api recordId;
              @api objectApiName;
              @track _omniSupportKey = 'cfMtxlpi_fc_contEduTableHeader_RenewalCouns';
              ...
              connectedCallback() {
                ...
                this.setAttribute(
                  "class",
                  "card-a6H5f000000LRPyEAO"
                );
                this.getCommunityPrefix().then((result) => {
                  loadStyle(this, result +"/servlet/servlet.FileDownload?file=00P5f00000190hPEAQ").then(() =>{
                    //Loading Custom CSS.
                  })
                });
...
data: this.handleEventAction.bind(this, data.events[1],1)
        ...
        pubsub.register(`mtxlpi_fc_contEduTableHeader_RenewalCouns`,this.pubsubEvent[0]);

              }

              unregisterEvents(){
                pubsub.unregister(`mtxlpi_fc_contEduTableHeader_RenewalCouns`,this.pubsubEvent[0]);
