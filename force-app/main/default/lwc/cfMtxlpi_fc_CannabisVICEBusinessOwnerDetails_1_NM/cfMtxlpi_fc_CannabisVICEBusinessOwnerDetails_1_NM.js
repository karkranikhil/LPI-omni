import { FlexCardMixin } from "vlocity_ins/flexCardMixin";
    import {interpolateWithRegex, interpolateKeyValue, fetchCustomLabels, loadCssFromStaticResource } from "vlocity_ins/flexCardUtility";
          ...
          export default class cfMtxlpi_fc_CannabisVICEBusinessOwnerDetails_1_NM extends FlexCardMixin(OmniscriptBaseMixin(LightningElement)){
              @api debug;
              @api recordId;
              @api objectApiName;
              @track _omniSupportKey = 'cfMtxlpi_fc_CannabisVICEBusinessOwnerDetails_1_NM';
        ...
        this.pubsubEvent[3] = {
          reload: this.handleEventAction.bind(this, data.events[3],3)
        };

        pubsub.register(`newreload`,this.pubsubEvent[3]);

        this.pubsubEvent[4] = {
          data: this.handleEventAction.bind(this, data.events[4],4)
        };

        pubsub.register(`mtxlpi_fc_CannabisPremisesDetails`,this.pubsubEvent[4]);
...
pubsub.unregister(`mtxlpi_fc_CannabisPremisesDetails`,this.pubsubEvent[4]);
