import { FlexCardMixin } from "vlocity_ins/flexCardMixin";
    import {interpolateWithRegex, interpolateKeyValue, fetchCustomLabels, loadCssFromStaticResource } from "vlocity_ins/flexCardUtility";
          ...
          export default class cfMtxlpi_fc_DisplaySupervisorListBNC extends FlexCardMixin(OmniscriptBaseMixin(LightningElement)){
              @api debug;
              @api recordId;
              @api objectApiName;
              @track _omniSupportKey = 'cfMtxlpi_fc_DisplaySupervisorListBNC';
        ...
        pubsub.register(`deleteSupervisor`,this.pubsubEvent[0]);

        this.pubsubEvent[1] = {
          data: this.handleEventAction.bind(this, data.events[1],1)
        };

        pubsub.register(`mtxlpi_fc_DisplaySupervisorList`,this.pubsubEvent[1]);

              }

              unregisterEvents(){
                pubsub.unregister(`deleteSupervisor`,this.pubsubEvent[0]);
pubsub.unregister(`mtxlpi_fc_DisplaySupervisorList`,this.pubsubEvent[1]);
