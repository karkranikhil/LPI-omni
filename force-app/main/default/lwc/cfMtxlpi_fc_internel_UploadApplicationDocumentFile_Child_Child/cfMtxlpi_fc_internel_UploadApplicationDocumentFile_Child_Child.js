import { FlexCardMixin } from "vlocity_ins/flexCardMixin";
    import {interpolateWithRegex, interpolateKeyValue, fetchCustomLabels, loadCssFromStaticResource } from "vlocity_ins/flexCardUtility";
          ...
          export default class cfMtxlpi_fc_internel_UploadApplicationDocumentFile_Child_Child extends FlexCardMixin(OmniscriptBaseMixin(LightningElement)){
              @api debug;
              @api recordId;
              @api objectApiName;
              @track _omniSupportKey = 'cfMtxlpi_fc_internel_UploadApplicationDocumentFile_Child_Child';
              ...
              connectedCallback() {
                ...
                this.setAttribute(
                  "class",
                  "card-a6H5f000000LROQEA4"
                );
                this.getCommunityPrefix().then((result) => {
                  loadStyle(this, result +"/servlet/servlet.FileDownload?file=00P5f00000190hlEAA").then(() =>{
                    //Loading Custom CSS.
                  })
                });
        ...
        pubsub.register(`mtxlpi_fc_uploadApplicationDocumentFile_Child_Child`,this.pubsubEvent[0]);

              }

              unregisterEvents(){
                pubsub.unregister(`mtxlpi_fc_uploadApplicationDocumentFile_Child_Child`,this.pubsubEvent[0]);
