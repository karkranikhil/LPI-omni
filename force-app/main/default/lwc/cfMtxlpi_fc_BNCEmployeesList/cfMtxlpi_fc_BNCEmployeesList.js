import { FlexCardMixin } from "vlocity_ins/flexCardMixin";
    import {interpolateWithRegex, interpolateKeyValue, fetchCustomLabels, loadCssFromStaticResource } from "vlocity_ins/flexCardUtility";
          ...
          export default class cfMtxlpi_fc_BNCEmployeesList extends FlexCardMixin(OmniscriptBaseMixin(LightningElement)){
              @api debug;
              @api recordId;
              @api objectApiName;
              @track _omniSupportKey = 'cfMtxlpi_fc_BNCEmployeesList';
        ...
        pubsub.register(`mtxlpi_fc_BNCEmployeesList`,this.pubsubEvent[1]);

              }

              unregisterEvents(){
                pubsub.unregister(`deleteRelatedLicense`,this.pubsubEvent[0]);
pubsub.unregister(`mtxlpi_fc_BNCEmployeesList`,this.pubsubEvent[1]);
