import { FlexCardMixin } from "vlocity_ins/flexCardMixin";
    import {interpolateWithRegex, interpolateKeyValue, fetchCustomLabels, loadCssFromStaticResource } from "vlocity_ins/flexCardUtility";
          ...
          export default class cfMtxlpi_fc_PersonExperienceList extends FlexCardMixin(OmniscriptBaseMixin(LightningElement)){
              @api debug;
              @api recordId;
              @api objectApiName;
              @track _omniSupportKey = 'cfMtxlpi_fc_PersonExperienceList';
        ...
        pubsub.register(`mtxlpi_fc_AddPersonExperience`,this.pubsubEvent[0]);

        this.pubsubEvent[1] = {
          data: this.handleEventAction.bind(this, data.events[1],1)
        };

        pubsub.register(`mtxlpi_fc_PersonExperienceList`,this.pubsubEvent[1]);

              }

              unregisterEvents(){
                pubsub.unregister(`mtxlpi_fc_AddPersonExperience`,this.pubsubEvent[0]);
pubsub.unregister(`mtxlpi_fc_PersonExperienceList`,this.pubsubEvent[1]);
