import { FlexCardMixin } from "vlocity_ins/flexCardMixin";
    import {interpolateWithRegex, interpolateKeyValue, fetchCustomLabels, loadCssFromStaticResource } from "vlocity_ins/flexCardUtility";
          ...
          export default class cfMtxlpi_fc_calculateTotalCEU_NewIntake extends FlexCardMixin(OmniscriptBaseMixin(LightningElement)){
              @api debug;
              @api recordId;
              @api objectApiName;
              @track _omniSupportKey = 'cfMtxlpi_fc_calculateTotalCEU_NewIntake';
        ...
        pubsub.register(`mtxlpi_fc_calculateTotalCEU_NewIntake`,this.pubsubEvent[0]);

              }

              unregisterEvents(){
                pubsub.unregister(`mtxlpi_fc_calculateTotalCEU_NewIntake`,this.pubsubEvent[0]);
