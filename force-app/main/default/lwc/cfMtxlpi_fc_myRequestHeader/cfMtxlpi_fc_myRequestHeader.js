import { FlexCardMixin } from "vlocity_ins/flexCardMixin";
    import {interpolateWithRegex, interpolateKeyValue, fetchCustomLabels, loadCssFromStaticResource } from "vlocity_ins/flexCardUtility";
          ...
          export default class cfMtxlpi_fc_myRequestHeader extends FlexCardMixin(OmniscriptBaseMixin(LightningElement)){
              @api debug;
              @api recordId;
              @api objectApiName;
              @track _omniSupportKey = 'cfMtxlpi_fc_myRequestHeader';
              ...
              connectedCallback() {
                ...
                this.setAttribute(
                  "class",
                  "card-a6H5f000000LRQLEA4"
                );
                this.getCommunityPrefix().then((result) => {
                  loadStyle(this, result +"/servlet/servlet.FileDownload?file=00P5f00000190wjEAA").then(() =>{
                    //Loading Custom CSS.
                  })
                });
