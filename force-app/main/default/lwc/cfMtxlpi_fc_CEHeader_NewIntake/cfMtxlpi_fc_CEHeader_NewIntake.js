import { FlexCardMixin } from "vlocity_ins/flexCardMixin";
    import {interpolateWithRegex, interpolateKeyValue, fetchCustomLabels, loadCssFromStaticResource } from "vlocity_ins/flexCardUtility";
          ...
          export default class cfMtxlpi_fc_CEHeader_NewIntake extends FlexCardMixin(OmniscriptBaseMixin(LightningElement)){
              @api debug;
              @api recordId;
              @api objectApiName;
              @track _omniSupportKey = 'cfMtxlpi_fc_CEHeader_NewIntake';
...
data: this.handleEventAction.bind(this, data.events[1],1)
        ...
        pubsub.register(`mtxlpi_fc_CEHeader_NewIntake`,this.pubsubEvent[0]);

              }

              unregisterEvents(){
                pubsub.unregister(`mtxlpi_fc_CEHeader_NewIntake`,this.pubsubEvent[0]);
