import { FlexCardMixin } from "vlocity_ins/flexCardMixin";
    import {interpolateWithRegex, interpolateKeyValue, fetchCustomLabels, loadCssFromStaticResource } from "vlocity_ins/flexCardUtility";
          ...
          export default class cfMtxlpi_fc_PresenterRelatedLicenseList extends FlexCardMixin(OmniscriptBaseMixin(LightningElement)){
              @api debug;
              @api recordId;
              @api objectApiName;
              @track _omniSupportKey = 'cfMtxlpi_fc_PresenterRelatedLicenseList';
        ...
        this.pubsubEvent[0] = {
          delete: this.handleEventAction.bind(this, data.events[0],0)
        };

        pubsub.register(`deleteRelatedLicense`,this.pubsubEvent[0]);

        this.pubsubEvent[1] = {
          data: this.handleEventAction.bind(this, data.events[1],1)
        };

        pubsub.register(`mtxlpi_fc_PresenterRelatedLicenseList`,this.pubsubEvent[1]);

              }

              unregisterEvents(){
                pubsub.unregister(`deleteRelatedLicense`,this.pubsubEvent[0]);
pubsub.unregister(`mtxlpi_fc_PresenterRelatedLicenseList`,this.pubsubEvent[1]);
