import { FlexCardMixin } from "vlocity_ins/flexCardMixin";
    import {interpolateWithRegex, interpolateKeyValue, fetchCustomLabels, loadCssFromStaticResource } from "vlocity_ins/flexCardUtility";
          ...
          export default class cfMtxlpi_fc_BNCStakeholderList extends FlexCardMixin(OmniscriptBaseMixin(LightningElement)){
              @api debug;
              @api recordId;
              @api objectApiName;
              @track _omniSupportKey = 'cfMtxlpi_fc_BNCStakeholderList';
        ...
        pubsub.register(`mtxlpi_fc_BNCStakeholderList`,this.pubsubEvent[1]);

              }

              unregisterEvents(){
                pubsub.unregister(`deleteRelatedLicense`,this.pubsubEvent[0]);
pubsub.unregister(`mtxlpi_fc_BNCStakeholderList`,this.pubsubEvent[1]);
